...
public class GuiManager {
    ...
    private backPanel logoHeader = null;
    ...
    public void GuiInit(){
        ...
        ResultSet resultSet = dbManager.dbSearch("Staff", "id", " where status=1");
        while (resultSet.next())
        {
            dbManager.dbUpdate("Staff", "status", "0", "id="+ resultSet.getString("id"));
        }
    ...
    public void GuiClose(){
        try{
            ...
            while (resultSet.next()){
                dbManager.dbUpdate("Staff", "status", "0", "id="+ resultSet.getString("id"));
                workThread.setInOutStatus(resultSet.getInt("id"));
            }
            dbManager.dbClose();
    ...
    private void setWindowTray(){
        ...
        //窗口最小化时软件dispose
        topContainer.addWindowListener(new WindowAdapter() {
            //图标化窗口时调用事件
            public void windowIconified(WindowEvent e) {
                topContainer.dispose(); //窗口最小化时dispose该窗口
            }
        });
        ...
        trayIcon.addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent e) {
                if (e.getClickCount() == 1) //双击托盘窗口再现
                    ...
                    topContainer.setExtendedState(Frame.NORMAL); //正常化状态
    ...
    public void createMainWindow(){
        topContainer = new JFrame("USTC振动控制与运动体控制科学实验室-考勤系统");
        ...
        topContainer.addWindowListener(new WindowListener() {
            @Override
        ...
        ImageIcon logo = new ImageIcon("UiResource/logoheader.jpg");
        logoHeader = new backPanel(logo);
        logoHeader.setLayout(new BorderLayout());
        logoHeader.add(createLogoHeader());

        logoHeader.paintComponents(logoHeader.getGraphics());
    ...
    private  JPanel createDetailSearch(){
        ...
        try {
            ...
            teamSelect = new JComboBox<String>(teamList);
...
//        resultStatis = createPersonalStatis(new String[]{"0", "0", "未达标", "0"});
    ...
    private JPanel createPersonMangager(){
        ...
        enroll.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent actionEvent) {
                if (userPemission <= MANAGER) {
                    createEnrollDialog();
                }else {
                    showErrorDialog("权限不足,无法打开");
                }
            }
        });
    ...
    private JPanel createSystemSet(){
        ...
        String[] timeList = new String[]{"06:00", "06:30", "07:00", "07:30", "08:00", "08:30", "09:00", "09:30", "10:00", "10:30", "11:00", "11:30", "12:00",
        ...
        try {
            componentManger.setSelectTime(start, end, timeList);
        }catch (Exception e){
            showErrorDialog(e.getMessage());
        }
        format.weightx = 1;
        ...
        save.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent actionEvent) {
                try {
                    if (userPemission > ROOT){
                        throw new Exception("权限不足,拒绝设置!");
                    }
                    if ((end.getSelectedIndex() - start.getSelectedIndex()) <= 0){
                        throw new Exception("请选择正确的时间范围");
                    }
                    componentManger.setCheckTime(start, end);
                }catch (Exception e){
                    showErrorDialog(e.getMessage());
                }
            }
        ...
        add.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent actionEvent) {
                try {
                    ...
                    componentManger.addTeam(groupContent);
                }catch (Exception e){
                    showErrorDialog(e.getMessage());
                }
            }
        ...
        delete.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent actionEvent) {
                try{
                    ...
                    componentManger.deleteTeam(groupContent);
                }catch (Exception e){
                    showErrorDialog(e.getMessage());
                }
            }
...
//                    //stopped!!!!
                    if (guiManager.userPemission > MANAGER) {
                        guiManager.showErrorDialog("权限不足,无法打开");
                    }else {
    ...
    public static void main(String args[]){
        try {
            GuiManager checkWindow = new GuiManager();
            checkWindow.GuiInit();
        }catch (Exception e){
            e.printStackTrace();
        }
